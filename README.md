...
Каждое решение осознанное, у каждого есть цена.

| Решение | Почему так | Цена |
|---|---|---|
| Субагенты с чистым контекстом, а не один длинный промпт | основной контекст остаётся лёгким, критик видит пробелы автора | больше токенов, сложнее оркестрация |
| Файлы как память, а не история чата | курс возобновляется из любой сессии, файлы открываются по ⌘+click | нужна дисциплина: диалог сразу материализуется в файл |
| Карта темы из внешних эталонов, а не из генерации | модель пишет план из усреднённой памяти и теряет разделы | ресёрч эталонов покрытия стоит времени |
| Автор и критик всегда разные агенты | самопроверка в своём контексте не ловит пробелы | лишние агенты и токены на проверку |
| Настройки-тумблеры, а не фиксированный режим | пользователь сам управляет ценой качества и стилем | можно недокрутить качество, если всё выключить |
| Один урок за раз, а не весь курс сразу | каждый следующий урок строится на разборе твоих ответов | нельзя нагенерить всё наперёд |

### Кастомизация

Это не чёрный ящик, почти всё правится файлами:

- **Шаблоны** в `references/templates/`: скелеты урока, заметки, профиля, MOC.
- **Палитра графа** в `obsidian-preset/graph.json`: меняешь `rgb` цветовых групп.
- **Стиль текста** в `references/style-rules.md`: свои запреты и предпочтения.
- **Дизайн-система** в `references/design-system.md`: callouts, направление схем, режимы оформления.
- **Глубина и язык** задаются полями профиля и переключаются по ходу.
- **Новая тема** добавляется модулями в существующие карту и план, а не новой папкой.
- **Свои источники** дописываются на гейте, помечаются как добавленные пользователем.
...
Кристина Винтер. Канал про продукты, ЗОЖ и мою весёлую жизнь: [t.me/vintersbrain](https://t.me/vintersbrain).
